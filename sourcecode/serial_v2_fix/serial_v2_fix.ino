  ...
  void loop() {
    while(!Serial.available() && !homeend)
    {
      Serial.println("Enter h to start homing.");
      delay(3000);
    }
    //동작을 시작
    while(Serial.available() || state.pre)
    { 
      while(!homeend)
      {
        if (Serial.read() == 'h')
      ...
      Serial.println("Press X, Y, Z to add.");
      Serial.println("Press x, y, z to subtract.");
      Serial.println("Press o for origin.");
        ...
        if(isDataExist)
        {
          ...
          for(int j = 0; j<maxStep; j++)
          {
            for(int i = 0; i<NUM_MOTOR; i++)
            {
              ...
              if(j<motor[i].steps.err) digitalWrite(motor[i].stepPin, HIGH);
